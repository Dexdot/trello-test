  ...
  actions: {
    ...
    async getBoards({ commit }) {
      try {
        ...
        const boards = json.map(board => ({ id: board.id, name: board.name, lists: [] }));
    ...
    async createBoard({ commit }, { name }) {
      try {
        ...
        commit("ADD_BOARD", { id: createdBoard.id, name: createdBoard.name, lists: [] });
      } catch (error) {
        console.error(error);
      }
    },
    ...
    async getBoardLists({ commit }, { boardID }) {
      try {
        ...
        const json = await response.json();
        const lists = json.map(list => ({ id: list.id, boardID: list.idBoard, name: list.name }));
    ...
    async createList({ commit }, { boardID, name }) {
      try {
        ...
        const list = { id: createdList.id, boardID: createdList.idBoard, name: createdList.name, tasks: [] };
    ...
    async getListTasks({ commit }, { boardID, listID }) {
      try {
        ...
        const json = await response.json();
        const tasks = json.map(task => ({ id: task.id, boardID: task.idBoard, listID: task.idList, name: task.name }));
    ...
    async createTask({ commit }, { listID, name }) {
      try {
        ...
        const task = { id: json.id, boardID, listID: json.idList, name: json.name };
